...
export class Collisions {
    static isOverlapPointVertices(point: number, vertices: number[]): number | false {
    ...
    static isOverlapVerticesVertices(a: number[], b: number[]): number | false {
        if (Math.min(...a) > Math.max(...b) || Math.min(...b) > Math.max(...a)) return false
        return Math.min(Math.max(...a) - Math.min(...b), Math.max(...b) - Math.min(...a))
    }

    static isOverlapPointVerticesDirection(point: Vector2, vertices: Vector2[], direction: Vector2): Vector2 | false {
        const pointProjection = point.dot(direction)
        const vertexProjections: number[] = []
    ...
    static isOverlapVerticesVerticesDirection(a: Vector2[], b: Vector2[], direction: Vector2): Vector2 | false {
    ...
    static isOverlap(point: Vector2, vertices: Vector2[]): Vector2 | false {
    ...
    static isOverlapVertices(a: Vector2[], b: Vector2[]): Vector2 | false {
        let shortestVector = new Vector2(Number.POSITIVE_INFINITY, Number.POSITIVE_INFINITY)
    ...
    static isCollisionPointGameObject(point: Vector2, gameObject: GameObject): Vector2 | false {
    ...
    static isCollisionGameObjectGameObject(a: GameObject, b: GameObject): Vector2 | false {
        const transformedA = a.getComponent(Collider)!.points.map(p =>
            Vector2.fromDOMPoint(a.transform.getWorldMatrix().transformPoint(p.toDOMPoint()))
        )
        const transformedB = b.getComponent(Collider)!.points.map(p =>
            Vector2.fromDOMPoint(b.transform.getWorldMatrix().transformPoint(p.toDOMPoint()))
        )
