...
/** Represents a collision pair with MTV result */
...
export class Scene {
    ...
    lastFrameMouseCollisions: GameObject[] = []
    lastFrameCollisions: CollisionPair[] = []
    previousMouseDowns: GameObject[] = []

    constructor(backgroundColor: string = "white") {
        const camera = this.instantiate(new GameObject("Camera"), new Vector2(0, 0))
        camera.addComponent(new Camera(), { backgroundColor })
    }
     ...
     */
    instantiate(gameObject: GameObject, position: Vector2 = new Vector2(0, 0)): GameObject {
    ...
    update(): void {
        ...
        let thisFrameMouseCollisions: GameObject[] = []
        const collidables = this.gameObjects.filter(go => go.getComponent(Collider))
        const rigidBodies = this.gameObjects.filter(go => go.getComponent(RigidBody))
        ...
        if (Input.mousePosition) {
            ...
            for (const collidable of collidables) {
                const mousePoint = collidable.layer === "UI"
                    ? Input.mousePosition.minus(new Vector2(Engine.letterBoxWidth / 2, Engine.letterBoxHeight / 2))
                    : mouse
                if (Collisions.isCollisionPointGameObject(mousePoint, collidable))
        ...
        const activeCollisions: CollisionPair[] = []
        const rigidBodyCollisions: CollisionPair[] = []

        for (let i = 0; i < collidables.length; i++) {
            for (let j = i + 1; j < collidables.length; j++) {
                if (
                    Engine.collisionLayers.length === 0 ||
                    Engine.collisionLayers.find(c =>
                        (c[0] === collidables[i].layer && c[1] === collidables[j].layer) ||
                        (c[0] === collidables[j].layer && c[1] === collidables[i].layer)
                    )
                ) {
                    ...
                    const collision: CollisionPair = one.id < two.id
                        ? { one, two, result }
                        : { one: two, two: one, result: result.times(-1) }
    ...
    draw(ctx: CanvasRenderingContext2D): void {
        ...
        if (cameraGO) {
            ctx.fillStyle = cameraGO.getComponent(Camera)!.backgroundColor
        ...
        if (cameraGO) {
            ...
            ctx.setTransform(ctx.getTransform().multiply(cameraGO.transform.getWorldMatrix().inverse()))
        }
 ...
 */
export function instantiate(gameObject: GameObject, position?: Vector2): GameObject {
